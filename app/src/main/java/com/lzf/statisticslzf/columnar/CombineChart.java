...
public class CombineChart extends View {
    ...
    //柱形图的颜色集合
    private int colors[] = new int[]{Color.RED, Color.parseColor("#78DA9F"), Color.parseColor("#FCAE84")}; //red代替了parseColor("#6FC5F4")
     ...
     * 背景的颜色
     */
    private static final int BG_COLOR = Color.WHITE; //WHITE代替了parseColor("#EEEEEE")
    /**
     * 向右边滑动的距离
     */
    ...
    private void init(Context context) {
        ...
        leftMargin = ScreenUtils.dp2px(context, 15); //手机右边的空白（统计图自己的左边）
        topMargin = ScreenUtils.dp2px(context, 20); //

        barPaint = new Paint();
        barPaint.setColor(colors[0]); //貌似不起作用

        axisPaint = new Paint();
        axisPaint.setStrokeWidth(2); //轴线的宽度（X轴、Y轴）
        axisPaint.setColor(Color.BLUE);//轴线的颜色（X轴、Y轴）
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (winds == null || mBarData == null) //|| humidity == null || temperature == null
            return;
        //重置3条线
        //        linePathW.reset();
    ...
    private void drawLeftYAxis(Canvas canvas) {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            canvas.drawLine(xStartIndex, startY, screenWidth - leftMargin, startY, axisPaint);
